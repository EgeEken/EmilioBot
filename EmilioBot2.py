...
while True:
    ...
    print("")
    context += message + "\n"
    input_context = torch.tensor([encode(context)], dtype=torch.long, device=device)
    response = parse_response(decode(model.generate(input_context, cutoff+50)[0].tolist())[len(context):], cutoff)
    context += response + "\n"
